...
int main(int argc, char* argv[]) {
    ...
    for (f::directory_entry file : f::directory_iterator(f::current_path())) {
        ...
        std::wstring filename = filepath.substr(filepath.find_last_of(L"\\") + 1, filepath.length());
    ...
    for (int fileNum = 0; fileNum < files.size(); fileNum++) {
        ...
        if (LONG_FORMAT) {
            std::wstring fileSize = HUMAN_READABLE ? humanSize(files[fileNum].size) : std::to_wstring(files[fileNum].size);
            content = ljust(files[fileNum].name, L' ', longestNameLength + MARGIN) 
                      + format_time(*files[fileNum].lastWrite) + ljust(L"", ' ', MARGIN)
                      + (files[fileNum].fileType == Directory ? L"" :fileSize) + L"\n";
        }
        ...
        std::wcout << std::format(L"\033[{}m{}\033[0m", colour, content);
    }
}
...
static std::wstring format_time(std::time_t time) {
    ...
    if (std::strftime(buffer, sizeof(buffer), "%b %d %Y %H:%M", &timeinfo)) {
        return std::wstring(&buffer[0], &buffer[99]);
    }
    else {
        return std::wstring{ L"000" };
    }
}
...
static std::wstring humanSize(uintmax_t bytes) {
    ...
    std::wstring unit = L"B"; // Default unit is bytes
    ...
    return std::format(L"{:.1f} {}", size_in_bytes, unit);
}